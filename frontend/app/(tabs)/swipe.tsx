    ...
    <SafeAreaView style={styles.root} edges={["top"]}>
      <View style={styles.header}>
        ...
        <View style={styles.counters}>
          ...
          <View style={styles.counterPillAccent} testID="superlike-counter">
            <Star size={14} color={colors.accent} fill={colors.accent} />
            <Text style={[styles.counterText, { color: colors.accent }]}>{superLikes}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.deck}>
        {loading ? (
          ...
          <>
            {next ? <CardView card={next} stacked colors={colors} styles={styles} /> : null}
            <GestureDetector gesture={pan}>
              <Animated.View style={[styles.card, cardStyle]} testID="swipe-card">
                <CardInner card={current} colors={colors} styles={styles} />
                ...
                <Animated.View style={[styles.overlayCenter, superOverlayStyle]}>
                  <Text style={[styles.overlayText, { color: colors.accent, fontSize: 48 }]}>SUPER</Text>
                </Animated.View>
              </Animated.View>
            </GestureDetector>
          </>
      ...
      {current ? (
        <View style={styles.actions}>
          ...
          <TouchableOpacity testID="superlike-btn" style={[styles.actionBtn, styles.superBtn]} onPress={() => superLikes > 0 && triggerAction("superlike")}>
            <Star size={26} color={colors.accent} fill={superLikes > 0 ? colors.accent : "transparent"} />
          </TouchableOpacity>
...
function CardView({ card, stacked, colors, styles }: { card: Card; stacked?: boolean; colors: ColorPalette; styles: any }) {
  return (
    <View style={[styles.card, stacked && { transform: [{ scale: 0.94 }, { translateY: 12 }], opacity: 0.6, position: "absolute" }]}>
      <CardInner card={card} colors={colors} styles={styles} />
    </View>
  );
}

function CardInner({ card, colors, styles }: { card: Card; colors: ColorPalette; styles: any }) {
  return (
    <View style={styles.cardInner}>
      ...
      <View style={styles.cardBody}>
        <View style={styles.statusRow}>
          <View style={[styles.statusDot, { backgroundColor: statusColorFn(card.activity_status, colors) }]} />
...
const makeStyles = (colors: ColorPalette) => StyleSheet.create({
  ...
  counterPill: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: colors.primarySoft, borderRadius: radius.pill, borderWidth: 1, borderColor: colors.borderStrong },
  counterPillAccent: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 10, paddingVertical: 6, backgroundColor: colors.accentSoft, borderRadius: radius.pill, borderWidth: 1, borderColor: colors.borderStrong },
  counterText: { color: colors.primary, fontSize: 13, fontWeight: "700" },
  ...
  matchPctBadge: { position: "absolute", top: 16, left: 16, flexDirection: "row", gap: 6, alignItems: "center", backgroundColor: colors.primarySoft, borderWidth: 1, borderColor: colors.primary, paddingHorizontal: 12, paddingVertical: 6, borderRadius: radius.pill },
  matchPctText: { color: colors.primary, fontSize: 13, fontWeight: "800" },
  statusRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  statusDot: { width: 8, height: 8, borderRadius: 4 },
  statusLabel: { color: "#E5DCD4", fontSize: 12, fontWeight: "600" },
  cardName: { color: "#FFFFFF", fontSize: 28, fontWeight: "800", letterSpacing: -0.5 },
  locRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  locText: { color: "#E5DCD4", fontSize: 13 },
  locDot: { color: "#A89890", marginHorizontal: 2 },
  bio: { color: "#F8F2EE", fontSize: 14, lineHeight: 20, marginTop: 4 },
  sharedRow: { marginTop: 10, gap: 6 },
  sharedLabel: { color: colors.primary, fontSize: 10, fontWeight: "800", letterSpacing: 1 },
  tagsRow: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  gameTag: { backgroundColor: "rgba(255,106,26,0.25)", borderWidth: 1, borderColor: colors.primary, paddingHorizontal: 10, paddingVertical: 4, borderRadius: radius.sm },
  gameTagText: { color: "#FFFFFF", fontSize: 12, fontWeight: "600" },
  overlay: { position: "absolute", top: 30, paddingVertical: 8, paddingHorizontal: 16, borderWidth: 3, borderRadius: 8 },
  likeOverlay: { right: 24, borderColor: colors.primary, transform: [{ rotate: "12deg" }] },
  passOverlay: { left: 24, borderColor: colors.pass, transform: [{ rotate: "-12deg" }] },
  overlayCenter: { position: "absolute", alignSelf: "center", top: "40%", borderWidth: 3, borderColor: colors.accent, paddingHorizontal: 24, paddingVertical: 8, borderRadius: 8 },
  overlayText: { fontSize: 38, fontWeight: "900", letterSpacing: 4, color: colors.primary },
  ...
  passBtn: { borderColor: colors.pass },
  superBtn: { borderColor: colors.accent, width: 52, height: 52, borderRadius: 26 },
  likeBtn: { borderColor: colors.primary },
  ...
  refreshBtn: { marginTop: spacing.md, backgroundColor: colors.primary, paddingVertical: 10, paddingHorizontal: 24, borderRadius: radius.pill },
